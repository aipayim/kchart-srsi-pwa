...
export const THRESH = {
  ...
  // ---- 强平根因预测（预防爆仓 v1.5.15）：真正能区分“会被强平”的开仓特征 ----
  // 归因结论(liqStudy)：强平=价格逆向走满 ~1/lev(≈13.9%@7x)，与 EMA120 背离几乎无关(0% 命中)；
  // 真正高命中因子是“追涨杀跌/接飞刀”——价格远离 1h EMA 或近根大振幅时开仓，强平率 15-18% vs 基线 12.7%。
  SMART_EMA1H_PCT: 3.0,           // 现价偏离 1h EMA 绝对值% ≥ 此值 → 接飞刀(强平率↑)
  SMART_CANDLE_PCT: 2.0,          // 近 4 根 15m 累计振幅% ≥ 此值(单向) → 追涨杀跌(强平率↑)

  // ---- 自适应杠杆 / 宽保护性止损（预防爆仓 v1.5.16+，实盘+回测共用，默认关） ----
  // 自适应杠杆：波动放大(当前 ATR% > 历史中位 ATR%)→ 降杠杆；平静→回到基准。
  // 爆仓距离 ∝ 1/杠杆，降杠杆直接把"需要的逆向幅度"拉大，使原本会爆的单够不到爆仓线。
  ADAPTIVE_LEV_MIN: 2,            // 自适应杠杆下限(倍数)；波动极端时也保底 2x
  ADAPTIVE_LEV_MED_N: 200,        // 计算中位 ATR% 用的历史窗口(根)；回测=全样本，实盘=滚动 EMA
  // 宽保护性止损：mult × 受监督 ATR% 作为价格止损距离%。默认 2×ATR ≈ 7x 爆仓距离(14.3%)的一半，
  // 落在爆仓线内侧——把灾难性 100% 爆仓压成有界小损，且因宽于正常噪声不被均值回归回撤频繁扫掉。
  ATR_STOP_MULT: 2.0,             // 宽止损倍数（×受监督 ATR%）

  // ---- 成交明细有界保留（v1.6.22）----
  // S.closed 是**用户数据**（不可被配额自愈清理），却每笔平仓 append 一条 → 无界增长，
  // 既威胁 localStorage 5MB 配额，也让盈亏页渲染成千上万行 DOM。这里统一保留最近 N 笔；
  // 头部统计不受影响：累计已实现盈亏走 S.realized，胜负数走 S.ai.w/l 与 sub.w。
  CLOSED_MAX: 2000,
};